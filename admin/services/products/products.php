<?php
try {
    $stmt = $connect->prepare("SELECT products.*, categories.category 
    FROM products LEFT JOIN categories ON products.category_id = categories.id ORDER BY products.code ASC");
    $stmt->execute();
