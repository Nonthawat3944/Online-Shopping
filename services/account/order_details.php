...
if (isset($_GET['order']) && $_GET['order'] == "cancel") {
    try {
        $stmt = $connect->prepare("UPDATE orders SET status = 5 WHERE id = :id");
        $stmt->execute(array(':id' => $_GET['id']));
?>
        <script>
            alert('ยกเลิกคำสั่งซื้อเรียบร้อย')
            window.location = '?account=order&order_details&code=<?= $_GET['code'] ?>'
        </script>
<?php
        ...
        <div class="d-flex justify-content-between align-items-center">
            ...
            <div class="text-status-date text-end">
                <strong class="d-block">สถานะ :
                    ...
                    ?>
                    <a href="?account=order&order_details&code=<?= $order['code'] ?>&order=cancel&id=<?= $order['id'] ?>" class="ms-2 btn btn-sm btn-danger <?= $order['status'] == 5 || $order['status'] == 3 || $order['status'] == 4 || $order['status'] == 6 ? 'disabled' : '' ?>" onclick="return confirm('ต้องการยกเลือกคำสั่งซื้อนี้ ? ')">ยกเลิกคำสั่งซื้อ</a>
                </strong>
